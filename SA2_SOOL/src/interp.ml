...
let lookup_class : string -> class_env -> class_decl ea_result  = fun c_name c_env ->
  match List.assoc_opt c_name c_env with                        (**List.assoc_opt -> iterate and search through a list of tuples *)
    | None -> error "lookup_class: class c_name not found"
    | Some (super,fields,methods) -> return (super,fields,methods)             (*e reps the triple *)
...
let lookup_method : string -> string -> class_env ->
  method_decl option = fun c_name m_name c_env ->
   match List.assoc_opt c_name c_env with                        
    | None -> None (** cant return error: "lookup_methods not found "cause its a class env and NOT an ea_result type *)
...
and
  ...
  | NewObject(c_name,args) ->
    mapM eval_expr args >>= fun evs -> (** should produce list of exp vals *)
    ...
    | None -> return self
    | Some m -> apply_method "initialize" self evs m >>= fun e -> 
                return self
    ) 

  | Send(e,m_name,args) ->
    eval_expr e >>= obj_of_objectVal >>= fun (c_name, env) ->
    let self = ObjectVal(c_name, env) in
    mapM eval_expr args >>= fun es ->
    ( match lookup_method c_name m_name !g_class_env with
      | None -> error "Method not found"
      | Some m -> apply_method m_name self es m 
    )
  | Self -> 
    eval_expr (Var "_self")
